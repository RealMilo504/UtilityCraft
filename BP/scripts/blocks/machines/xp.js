...
world.beforeEvents.worldInitialize.subscribe(eventData => {
    eventData.blockComponentRegistry.registerCustomComponent('utilitycraft:xp', {
        onPlayerInteract(e) {
            const { block } = e
            if (block.typeId == 'utilitycraft:xp_spout') {
                if (block.permutation.getState('utilitycraft:isOpen') == false) {
                    block.setPermutation(block.permutation.withState('utilitycraft:isOpen', true))
                } else { block.setPermutation(block.permutation.withState('utilitycraft:isOpen', false)) }
            }
        },
        onTick(e) {
            const { block } = e
            ...
            if (block.typeId == 'utilitycraft:xp_drain') {
                ...
                if (player != undefined && tankEntity == undefined && tank.permutation.getState('utilitycraft:hasliquid') == false) {
                    if (player.getTotalXp() > 0) {
                        tank.setPermutation(tank.permutation.withState('utilitycraft:hasliquid', true))
                        y -= 1
                        block.dimension.spawnEntity(`utilitycraft:fluid_tank_xp`, { x, y, z })
                        block.dimension.runCommandAsync(`tag @e[type=utilitycraft:fluid_tank_xp] add tank`)
                        system.run(() => {
                ...
                if (player != undefined && tankEntity != undefined && tank.permutation.getState('utilitycraft:hasliquid') == true) {
                    ...
                    if (player.getTotalXp() > 0 && actualXp < tankCap.effectiveMax) {
                        ...
                        if (player.xpEarnedAtCurrentLevel == 0) {
                            ...
                            tankEntity.setDynamicProperty('utilitycraft:liquid', actualXp + 1);
                        } else {
                            ...
                            tankEntity.setDynamicProperty('utilitycraft:liquid', actualXp + amount);
                        }
                    }
                }
            }

            if (block.typeId == 'utilitycraft:xp_spout' && block.permutation.getState('utilitycraft:isOpen') == true) {
                let xs = 0, zs = 0
                let tank = undefined
                ...
                if (tankEntity != undefined) {
                    if (tankEntity.typeId != 'utilitycraft:fluid_tank_xp') return
                    ...
                    if (tankCap > 1) {
                        ...
                        tankEntity.remove()
                        block.dimension.spawnEntity('xp_orb', { x: block.location.x + xs, y: block.location.y + 0.5, z: block.location.z + zs })
                        tank.setPermutation(tank.permutation.withState('utilitycraft:hasliquid', false))
                    }
                }
            }
        }
